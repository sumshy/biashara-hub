...
function Details() {
  ...
  const fetchMerchantsDetails = async () => {
    try {
      ...
      const response = await fetch(
        "https://biashara-hub.onrender.com/api/client/getServiceProvider",
        {
          method: "POST",
          headers: {
  ...
  const fetchMerchantsProducts = async () => {
    try {
      const token = getAuthToken();
      const response = await fetch(
        "http://localhost:5000/api/client/getProviderProducts/?pageNumber=1&limit=3",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchOtherProducts = async () => {
    try {
      const token = getAuthToken();
      const response = await fetch(
        `http://localhost:5000/api/client/getProductsByCategory/?category=${category}&pageNumber=1&limit=6`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
        ...
        <div className="info1">
          ...
          <div className="column">
            ...
            <div className="contact-info">
              <p>
                <FontAwesomeIcon icon={faPhone} /> {merchant.provider?.phone}
              </p>
              <p>
                <FontAwesomeIcon icon={faEnvelope} /> {merchant.provider?.email}
              </p>
              <p>
                <FontAwesomeIcon icon={faMapMarkerAlt} />{" "}
                {merchant.provider?.location}
              </p>
            </div>
          </div>
          <div className="column">
            <div className="card">
              ...
              <button className="book-service" onClick={handleBookServiceClick}>
                Book Service
              </button>
            </div>
          </div>
          ...
          <div className="column">
            ...
            <div className="other-service">
              {otherProducts.map((service) => (
                <ServiceCard key={service.id} title={service?.name} />
              ))}
            </div>
          </div>
        </div>
